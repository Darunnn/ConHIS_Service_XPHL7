...
{
    public class HL7DetailForm : Form
    {
        ...
        private void SetupUI()
        {
            ...
            // สร้าง 8 tabs
            tabControl.TabPages.Add("MSH", "MSH - Message Header");
            tabControl.TabPages.Add("PID", "PID - Patient ID");
            tabControl.TabPages.Add("PV1", "PV1 - Patient Visit");
            tabControl.TabPages.Add("ORC", "ORC - Common Order");
            tabControl.TabPages.Add("AL1", "AL1 - Allergies");
            tabControl.TabPages.Add("RXD", "RXD - Pharmacy Dispense");
            tabControl.TabPages.Add("RXR", "RXR - Route Info");
            tabControl.TabPages.Add("NTE", "NTE - Notes");
        ...
        private void LoadData()
        {
            ...
            tabControl.TabPages["MSH"].Controls.Add(mshGrid);
            LoadMSH(mshGrid);

            // PID Tab
            var pidGrid = CreateDataGridView();
            tabControl.TabPages["PID"].Controls.Add(pidGrid);
            LoadPID(pidGrid);

            // PV1 Tab
            var pv1Grid = CreateDataGridView();
            tabControl.TabPages["PV1"].Controls.Add(pv1Grid);
            LoadPV1(pv1Grid);

            // ORC Tab
            var orcGrid = CreateDataGridView();
            tabControl.TabPages["ORC"].Controls.Add(orcGrid);
            LoadORC(orcGrid);

            // AL1 Tab
            var al1Grid = CreateDataGridView();
            tabControl.TabPages["AL1"].Controls.Add(al1Grid);
            LoadAL1(al1Grid);

            // RXD Tab
            var rxdGrid = CreateDataGridView();
            tabControl.TabPages["RXD"].Controls.Add(rxdGrid);
            LoadRXD(rxdGrid);

            // RXR Tab
            var rxrGrid = CreateDataGridView();
            tabControl.TabPages["RXR"].Controls.Add(rxrGrid);
            LoadRXR(rxrGrid);

            // NTE Tab
            var nteGrid = CreateDataGridView();
            tabControl.TabPages["NTE"].Controls.Add(nteGrid);
            LoadNTE(nteGrid);
        }
