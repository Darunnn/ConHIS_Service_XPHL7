...
namespace ConHIS_Service_XPHL7.Services
{
    public class HL7Service
    {
        ...
        #region Generate HL7 Message
        public string GenerateHL7Message(HL7Message message)
        {
            var sb = new StringBuilder();

            if (message.MessageHeader != null) sb.AppendLine(GenerateMSH(message.MessageHeader));
            if (message.PatientIdentification != null) sb.AppendLine(GeneratePID(message.PatientIdentification));
        ...
        private string GeneratePID(PID pid)
        {
            // OfficialName PID-5
            string officialName = $"{pid.OfficialName.LastName}^{pid.OfficialName.FirstName}^{pid.OfficialName.MiddleName}^{pid.OfficialName.Suffix}^{pid.OfficialName.Prefix}^{pid.OfficialName.Degree}^{pid.OfficialName.NameTypeCode}^{pid.OfficialName.NameRepresentationCode}";
            // AliasName PID-9
            string aliasName = $"{pid.AliasName.LastName}^{pid.AliasName.FirstName}^{pid.AliasName.MiddleName}^{pid.AliasName.Suffix}^{pid.AliasName.Prefix}^{pid.AliasName.Degree}^{pid.AliasName.NameTypeCode}^{pid.AliasName.NameRepresentationCode}";
            // Address PID-11
            string address = $"{pid.Address.StreetAddress}^{pid.Address.OtherDesignation}^{pid.Address.City}^{pid.Address.StateOrProvince}^{pid.Address.ZipOrPostalCode}";

            return $"PID|{pid.SetID}|{pid.PatientIDExternal}|{pid.PatientIDInternal}|{pid.AlternatePatientID}|{officialName}|{pid.Mothermaidenname}|{FormatDateTime(pid.DateOfBirth)}|{pid.Sex}|{aliasName}|{pid.Race}|{address}|{pid.Country}|{pid.PhoneNumberHome}";
        }
        ...
        private string GeneratePV1(PV1 pv1)
        {
            ...
            // PatientType PV1-18
            string patientType = $"{pv1.PatientType.ID}^{pv1.PatientType.Name}";

            return $"PV1|{pv1.SetID}|{pv1.PatientClass}|{location}||||||||||||||||" +
                   $"{admittingDoctor}|{patientType}|||||||||||||||||||||||||||||||";
        }


        private string GenerateORC(ORC orc)
        {
            ...
            return $"ORC|{orc.OrderControl}|{orc.PlacerOrderNumber}|{orc.FillerOrderNumber}|{orc.PlacerGroupID}|{orc.OrderStatus}|" +
                   ...
                   $"{orc.OrderingFacilityName}|{orc.OrderingFacilityAddress}";
        }


        private string GenerateAL1(AL1 al1)
        {
        ...
        private string GenerateRXR(RXR rxr)
        {
            return $"RXR|{rxr.Route}|{rxr.site}|{rxr.AdministrationDevice}|{rxr.AdministrationMethod}|{rxr.RoutingInstruction}";
        }


        private string GenerateNTE(NTE nte)
        {
            return $"NTE|{nte.SetID}|{nte.CommentType}|{nte.CommentNote}";
        }
        #endregion
